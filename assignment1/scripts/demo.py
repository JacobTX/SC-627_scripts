...
d_crit = 0.5 # critical distance to avoid collision
d_reached = 0.01 # critical distance to conclude that bot has reached the goal location
w0 = 0.4  # characteristic value of angular velocity
...
def laserdata_callback(msg):
    ...
    if (d0 > d_crit):
        ...
        move_the_bot.angular.z = 0.1*sgn(d45-d135)
...
if __name__ == "__main__":
    ...
    subscribe_to_laser = rospy.Subscriber('/tb3_2/scan', LaserScan, callback = laserdata_callback)
    rospy.loginfo('My node has been started')
    publish_to_cmd_vel = rospy.Publisher('/tb3_2/cmd_vel', Twist, queue_size = 10)

    #subscribe to odometry data to get the bot's pose
    subscribe_to_odom = rospy.Subscriber('/tb3_2/odom',Odometry, callback = odomdata_callback) 
