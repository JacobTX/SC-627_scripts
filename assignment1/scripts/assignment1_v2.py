...
bot_list_x = [] # list that stores the x-coordinates of the bot
bot_list_y = [] # list that stores the y-coordinates of the bot
dist_list = [] # list that stores the bot-goal distance
...
def odomdata_callback(data):
    ...
    global bot_pose, bot_list_x, bot_list_y, goal_loc
    bot_pose = [xloc,yloc,theta]

    bot_list_x.append(xloc)
    bot_list_y.append(yloc) 
    dist_list.append(distance(bot_pose,goal_loc))

def bot_loc_plot(A,B):
    plt.plot(A,B,'b')
    plt.xlabel('Bot x-coordinate')
    plt.ylabel('Bot y-coordinate')
    plt.title('Plot of Bot location')
    plt.show()

def dist_plot(A,B):
    plt.plot(A,B,'b')
    plt.xlabel('Time')
    plt.ylabel('Bot-goal distance')
    plt.title('Plot of Bot-goal distance vs time')
    plt.show()

def laserdata_callback(msg):

    
    global min_dist, min_loc, start_loc, d_crit, d_reached, direc, w0, v0, s, c, n
    ...
    if distance(bot_pose,goal_loc) < d_reached :
        ...
        #bot_loc_plot(bot_list_x,bot_list_y)
        time = range(len(dist_list))
        dist_plot(time,dist_list)
        ...
        if (c==0):
            ...
            if distance(start_loc,bot_pose) > 0.3:
                n = 1 #First loop started
            
            if distance(start_loc,bot_pose) < 0.2 and (n==1):
